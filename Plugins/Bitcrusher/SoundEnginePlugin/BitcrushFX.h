...
public:
    ...
    // Reduce bit depth of a given sample
    AkReal32 BitReduction(AkReal32 InputSample, AkReal32 BitRate);

    // Downsample by returning every Nth (index) sample
    AkReal32 DownSample(AkReal32 InputSample, AkReal32 Factor);

    // Hard clip signal by limiting output by a given threshold
    AkReal32 HardClip(AkReal32 InputSample, AkReal32 Threshold);

    // Arctangent soft clipping with a given drive amount
    AkReal32 SoftClip(AkReal32 InputSample, AkReal32 Alpha);
...
private:
    BitcrushFXParams* m_pParams;
    AK::IAkPluginMemAlloc* m_pAllocator;
    AK::IAkEffectPluginContext* m_pContext;

    // Downsample/foldover memory variables
    AkReal32 Index;
    size_t SampleIndex;
    AkReal32 PreviousSample;
};
